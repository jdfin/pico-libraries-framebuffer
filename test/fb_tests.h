...
// Shared test/demo suite for every display driver's test executable
// (ws35_test.cpp, ws24_test.cpp, and the Newhaven/4D-Systems boards'
// equivalents). This is included, not compiled as its own translation
// unit: several tests (ImgButton, Nav, ...) build PixelImage<> glyph
// bitmaps at compile time sized from fb_width/fb_height, and other tests
// pick a board-appropriate Font, so this file needs those as genuine
// compile-time constants rather than runtime/link-time values. Each
// board's thin test .cpp defines them, then does:
//
//   #include "fb_tests.h"
//
// after which it can call fb_tests_run(fb) from main(). Every name in
// this file is 'static' (or inside an anonymous-linkage namespace), so
// each including .cpp gets its own independent copy - no ODR conflicts
// between the several executables that include this header.
//
// Board-specific names this file expects to already be declared:
//   fb_width, fb_height  - landscape logical size (int)
//   font                 - default text font (const Font &)
//   nav_font, id_font, toots_font, slider_font
//                        - per-namespace font picks scaled to this board
//                          (see the Nav/Id/Toots/Slider namespaces below)
...
// Entry point called from each board's main() once the display is
// initialized, rotated, and backlit: prints the test menu and dispatches
// entered test numbers until reset.
...
static const Font &font = nav_font; // ~fb_height/11.5
...
static const Font &font = id_font; // ~fb_height/6.67
...
static const Font &font = toots_font; // ~fb_height/9.4
...
static const Font &font = slider_font; // ~fb_height/9.4
static constexpr int hgtx = 5;          // extra height above and below text
static constexpr int hgt = font.y_adv + 2 * hgtx;
static constexpr int marg = 1;                  // distance from edges of screen
